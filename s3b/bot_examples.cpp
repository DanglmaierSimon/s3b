...
namespace sc2 {
	...
	// Tries to find a random location that can be pathed to on the map.
	// Returns 'true' if a new, random location has been found that is pathable by the unit.
	bool TerranBot::FindEnemyPosition(Point2D& target_pos, int loop) {
		if (game_info_.enemy_start_locations.empty()) {
			TryMoveRandomUnit();
			return false;
		}
		...
		auto sl = game_info_.enemy_start_locations;
		target_pos = sl.at(loop % game_info_.enemy_start_locations.size());
		return true;
	}

	void TerranBot::ScoutWithMarines(int loop) {
		...
		for (const auto& unit : units) {
			...
			if (FindEnemyStructure(Observation(), enemy_unit)) {
				assert(enemy_unit);
				Actions()->UnitCommand(unit, ABILITY_ID::ATTACK, enemy_unit->pos);
				return;
			}

			Point2D target_pos;
			// TODO: For efficiency, these queries should be batched.
			if (FindEnemyPosition(target_pos, Observation()->GetGameLoop())) {
				Actions()->UnitCommand(unit, ABILITY_ID::SMART, target_pos);
			}
		}
	}

	static int targetScvCount(const ObservationInterface* obs) {
		auto filter = [](const Unit& unit) -> bool { return unit.unit_type.ToType() == UNIT_TYPEID::TERRAN_COMMANDCENTER; };
		auto ccs = obs->GetUnits(Unit::Alliance::Self, filter).size();

		return std::min(ccs * 16, (size_t)80);
	}
	...
	bool TerranBot::TryBuildSupplyDepot() {
		...
		auto cost = observation->GetUnitTypeData().at(static_cast<uint32_t>(UNIT_TYPEID::TERRAN_SUPPLYDEPOT)).mineral_cost;

		if (observation->GetMinerals() < cost) {
			return false;
		}
	...
	bool TerranBot::TryBuildBarracks() {
		...
		if (CountUnitType(observation, UNIT_TYPEID::TERRAN_SCV) < (targetScvCount(observation) * 2 / 3))
			return false;
	...
	bool TerranBot::TryBuildSCV() {
		if (CountUnitType(Observation(), UNIT_TYPEID::TERRAN_SCV) >= targetScvCount(Observation()))
			return false;
	...
	const Unit* TerranBot::FindNearestMineralPatch(const Point2D& start) {
		Units units = Observation()->GetUnits(Unit::Alliance::Neutral);
		float distance = std::numeric_limits<float>::max();
		const Unit* target = nullptr;
		for (const auto& u : units) {
	...
	void TerranBot::OnStep() {
		...
		ScoutWithMarines(Observation()->GetGameLoop());
	...
	void TerranBot::OnUnitIdle(const Unit* unit) {
		if (unit->unit_type == UNIT_TYPEID::TERRAN_SCV) {
			auto minpatch = FindNearestMineralPatch(unit->pos);
			if (minpatch) {
				Actions()->UnitCommand(unit, ABILITY_ID::HARVEST_GATHER, minpatch);
			}
		}
	}
