...
private:
  ...
  inline bool CanPathToLocation(const sc2::Unit *unit, sc2::Point2D &target_pos) {
    // Send a pathing query from the unit to that point. Can also query from
    // point to point, but using a unit tag wherever possible will be more
    // accurate. Note: This query must communicate with the game to get a result
    // which affects performance. Ideally batch up the queries (using
    // PathingDistanceBatched) and do many at once.
  ...
  inline bool train(const ObservationInterface *obs, sc2::ActionInterface *actions, sc2::QueryInterface *queries,
                    UNIT_TYPEID type, int amount = 1, bool allow_queueing = true) {
    using namespace std;
    auto p_units = get_production_unit(type);

    (void)allow_queueing; // TODO Implement queueing of units -> is this even
                          // required?

    if (p_units.empty() || p_units[0] == UNIT_TYPEID::INVALID) {
      std::cout << "WARNING: Requested builders for " << UnitTypeToName(UnitTypeID(type)) << "(" << UnitTypeID(type) << ")"
                << " and received none!" << std::endl;
      return false;
    }

    auto builder_filter = [&p_units](const Unit &unit) -> bool {
      return unit.alliance == Unit::Alliance::Self && unit.build_progress == 1.0 && IsUnits{p_units}(unit);
    };
    ...
    while (left_to_train > 0 && have_enough_supply(obs, unit_data[(uint32_t)type].food_required)) {
      ...
      if (builder->orders.empty()) {
        ...
        std::cout << "INFO: Sending command " << AbilityTypeToName(AbilityID(ability)) << "; " << AbilityID(ability)
                  << " to unit " << UnitTypeToName(builder->unit_type) << "; " << UnitTypeID(builder->unit_type) << std::endl;
  ...
  inline bool can_afford(const ObservationInterface *obs, UNIT_TYPEID type) {
    ...
    if (obs->GetMinerals() >= mineral_cost && obs->GetVespene() >= gas_cost && (have_enough_supply(obs, supply_cost))) {
      return true;
    } else {
      return false;
    }
